...
for jj_, jj in enumerate(jjs):
    ...
    Cl[jj_] = result*1./np.pi**2/2.*prefac**2/2.*-2. 
    Chis[jj_] = chi
...
if rank ==0:
    ...
    print(chis.shape)
    np.save('../G_matrices/clpsiphi_parallel_magbias_%s.npy'%file_ext,cl)
    np.save('../G_matrices/clpsiphi_parallel_magbias_chis_%s.npy'%file_ext,chis)
